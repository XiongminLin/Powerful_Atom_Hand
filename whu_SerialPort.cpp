...
// 打开串口
BOOL whu_SerialPort::Open ( int nPort, int nBaud )
{
	if ( m_bOpened )
		return TRUE;
...
// 关闭串口
BOOL whu_SerialPort::Close ( void )
{
...
// 向串口写入数据，类内部使用
BOOL whu_SerialPort::WriteCommByte ( unsigned char ucByte )
{
	BOOL bWriteStat;
...
// 向串口发送数据
int whu_SerialPort::SendData ( const char *buffer, int size )
{
...
int whu_SerialPort::ReadDataWaiting ( void )
{
	if ( !m_bOpened || m_hIDComDev == NULL ) return ( 0 );
...
// 读取来自串口的数据
int whu_SerialPort::ReadData ( void *buffer, int limit )
{
